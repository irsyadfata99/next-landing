      ...
      <div className="flex transition-transform ease-out duration-500" style={{ transform: `translateX(-${currentSlide * 100}%)` }}>
        {Array.isArray(slides) &&
          slides.map((slide, index) => (
            <div key={index} className="carousel-slide w-full h-full flex flex-col justify-center items-center">
              <Image src={slide.image} alt="carousel-img" width={457} height={327} className="rounded-2xl object-cover border-8 border-primary" />
              <p className="text-xl font-semibold mt-4">{slide.title}</p>
            </div>
          ))}
      </div>
      <div className="z-10 inset-0 flex items-center justify-between pt-6">
